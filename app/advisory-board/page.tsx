...
const web3Board = [
  { name: 'Advisor Web3', role: 'Stratégie protocole & écosystème', image: 'W3', linkedin: '#', bio: 'Profil à compléter avec une expertise protocoles Web3, gouvernance, écosystèmes et mécanismes de financement.', focus: ['Stratégie protocole', 'Gouvernance', 'Écosystème'] },
  { name: 'Advisor systèmes distribués', role: 'Infrastructure blockchain', image: 'DS', linkedin: '#', bio: 'Profil à compléter avec une expertise en systèmes distribués, infrastructure blockchain et passage à l’échelle opérationnel.', focus: ['Infrastructure', 'Scalabilité', 'Vérification'] },
  { name: 'Advisor IA x Web3', role: 'Systèmes agentiques & infrastructure décentralisée', image: 'AI', linkedin: '#', bio: 'Profil à compléter avec une expertise à l’intersection des agents IA, des systèmes décentralisés et de l’exécution de confiance.', focus: ['Agents IA', 'Web3', 'Trust layers'] },
]
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-7xl px-6 py-24 text-white">
        ...
        <section className="mt-28"><div className="flex items-end justify-between gap-6"><div><span className="text-sm uppercase tracking-[0.3em] text-primary">Écosystème Web3</span><h2 className="mt-4 font-display text-4xl font-semibold">Web3 Advisory Board</h2></div><div className="hidden rounded-full border border-white/10 bg-white/[0.03] px-5 py-2 text-sm text-zinc-400 md:block">Profils en cours</div></div><div className="mt-10 grid gap-6 md:grid-cols-2 xl:grid-cols-3">{web3Board.map((member) => <AdvisoryCard key={member.name} member={member} />)}</div></section>
      </main>
      <FinalCTAFr />
      <Footer />
    </>
